...
class ClsOnlineEWCModel(OnlineEWCModel):
    def get_trained_params(self):
        qkv_and_norm_params = [p for n, p in self.models_dict['main'].named_parameters() if 'head' in n]
        return qkv_and_norm_params
    
    def forward_to_get_task_loss(self, x, y, offset, cur_num_cls):
        return F.cross_entropy(self.infer(x)[:, 20: offset + cur_num_cls], y - 20) # TODO: debug
    ...
    def get_accuracy(self, test_loader, *args, **kwargs):
        ...
        cur_class_offset_end = self.cur_class_offset
        source_class_offset = self.source_class_offset
        ...
        with torch.no_grad():
            ...
            for batch_index, (x, y) in pbar:
                ...
                output = output[:, source_class_offset: cur_class_offset_end]
                if batch_index == 0:
                    print(source_class_offset, cur_class_offset_end)
                    print(output[0:3])
                    print(self.models_dict['main'].head.weight[source_class_offset: ])
                y = y - source_class_offset
