...
class ElasticDNN_VQAOnlineModel(ElasticDNN_OnlineModel):
    ...
    def add_cls_in_head(self, num_cls):
        head: nn.Linear = get_module(self.models_dict['md'], 'classifier.3')
        ...
        set_module(self.models_dict['md'], 'classifier.3', new_head)
        set_module(self.models_dict['fm'], 'classifier.3', new_head)
    ...
    def get_md_matched_param_of_sd_param(self, sd_param_name):
        ...
        if p.dim() == 0:
            return None
        elif p.dim() == 1 and ('LayerNorm' in self_param_name and 'layernorm' in self_param_name) and 'weight' in self_param_name:
...
from methods.gem.gem_el_vilt import OnlineGEMModel
import tqdm
from methods.feat_align.mmd import mmd_rbf
from copy import deepcopy


class VQAOnlineGEMModel(OnlineGEMModel):
