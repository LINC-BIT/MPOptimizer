...
class ElasticDNN_Roberta_OfflineSenClsMDModel(ElasticDNN_OfflineSenClsMDModel):
    ...
    def get_matched_param_of_fm(self, self_param_name, fm: nn.Module): # TODO:
        if any([k in self_param_name for k in ['fbs', 'embeddings', 'ln']]):
            return None
...
if __name__ == '__main__':
    ...
    md_models_dict_path = save_models_dict_for_init(
        torch.load('new_impl/nlp/roberta/sentiment-classification/results/cls_md_wo_fbs.py/20240106/999999-134324/models/md_best.pt', map_location=torch.device('cuda:1')), 
        __file__, 'md_roberta_sen_cls_raw_pretrained')
    device = 'cuda:1'
    ...
    fm_to_md_alg.run(scenario, hyps={
        ...
        'num_workers': 32,
        'optimizer': 'AdamW',
        'optimizer_args': {'lr': 1e-5, 'betas': [0.9, 0.999]},
        'scheduler': 'LambdaLR',
