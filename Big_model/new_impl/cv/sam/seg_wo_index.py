...
class ElasticDNN_sam_OfflineSegMDModel(ElasticDNN_OfflineSegMDModel):
    ...
    #     else:
    #         return get_parameter(fm, self_param_name)
        if 'attn.qkv' in self_param_name and ('weight' in self_param_name):
            ss = self_param_name.split('.')
        ...
        elif 'attn.qkv' in self_param_name and ('bias' in self_param_name):
            ss = self_param_name.split('.')
...
if __name__ == '__main__':
    ...
    fm_models_dict_path = 'new_impl/cv/sam/results/seg_wo_fbs.py/20231125/999997-111727/models/fm_best.pt'
    fm_models_dict_path = save_models_dict_for_init(torch.load(fm_models_dict_path), __file__, 'fm_sam_cls_lora')
    pretrained_md_models_dict_path = 'new_impl/cv/sam/results/seg_wo_fbs.py/20231125/999997-111727/models/md_best.pt'
    md_models_dict = torch.load(pretrained_md_models_dict_path)
    md_models_dict_path = save_models_dict_for_init(md_models_dict, __file__, 'md_sam_cls_pretrained_wo_fbs')
    torch.cuda.set_device(1)
    device = 'cuda'
    
    fm_model = ElasticDNN_sam_OfflineSegFMModel('fm', fm_models_dict_path, device, scenario.num_classes)
    md_model = ElasticDNN_sam_OfflineSegMDModel('md', md_models_dict_path, device,scenario.num_classes)
