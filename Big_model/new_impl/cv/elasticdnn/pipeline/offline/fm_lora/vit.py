...
class FMLoRA_ViT_Util(FMLoRA_Util):
    ...
    def add_lora_ab_to_fm(self, fm: nn.Module, ab_r: int, samples: torch.Tensor):
        ...
        for name, module in fm.named_modules():
            ...
            #if not name.endswith('.projection_key') or not name.endswith('.projection_query') or not name.endswith('.projection_value'):
            if name.endswith('.projection_query') or name.endswith('.projection_key') or name.endswith('.projection_value'):
