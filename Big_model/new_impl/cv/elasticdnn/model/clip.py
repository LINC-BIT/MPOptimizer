...
class ElasticCLIPUtil(ElasticDNNUtil):
    def convert_raw_dnn_to_master_dnn(self, raw_dnn: nn.Module, r: float, ignore_layers=[]):
        ...
        for name, module in raw_vit.named_modules():
            ...
            if 'vision_model' in name and name.endswith('mlp'):
    ...
    def extract_surrogate_dnn_via_samples(self, master_dnn: nn.Module, samples: torch.Tensor, return_detail=False):
        ...
        # for block_i, block in enumerate(boosted_vit.model.text_model.encoder.layers):
        #     # attn = block.attn
        #     # ff = block.mlp
        ...
        #     unpruned_indexes_of_layers[f'boosted_vit.model.text_model.encoder.layers.{block_i}.mlp.fc1.weight'] = ff_0_unpruned_indexes
            
            
        for block_i, block in enumerate(boosted_vit.model.vision_model.encoder.layers):
            # attn = block.attn
            # ff = block.mlp
            ...
            unpruned_indexes_of_layers[f'boosted_vit.model.vision_model.encoder.layers.{block_i}.mlp.fc1.weight'] = ff_0_unpruned_indexes
