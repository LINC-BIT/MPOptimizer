...
class ElasticresUtil(ElasticDNNUtil):
    def convert_raw_dnn_to_master_dnn(self, raw_dnn: nn.Module, r: float, ignore_layers=[]):
        ...
        for name, module in raw_vit.named_modules():
            ...
            if name.endswith('convolution'):
                set_module(module,'',ProjConv_WrappedWithFBS(module,r))
                break
    ...
    def extract_surrogate_dnn_via_samples(self, master_dnn: nn.Module, samples: torch.Tensor, return_detail=False):#产生小模型的步骤
        ...
        # return boosted_vit
        from new_impl.cv.resnet.extract_submodel import FBSSubModelExtractor,DAFBSSubModelExtractor
        x = DAFBSSubModelExtractor()
        boost, indexes = x.extract_submodel_via_samples(master_dnn,samples)
        return boost , indexes
