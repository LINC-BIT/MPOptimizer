...
class ElasticDNN_clip_OfflineClsFMModel(ElasticDNN_OfflineClsFMModel):
...
class ElasticDNN_clip_OfflineClsMDModel(ElasticDNN_OfflineClsMDModel):
    ...
    #     else:
    #         return get_parameter(fm, self_param_name)
        if ('self_attn.q_proj' in self_param_name or 'self_attn.k_proj' in self_param_name or \
            'self_attn.v_proj' in self_param_name) and ('weight' in self_param_name):
            ss = self_param_name.split('.')
        ...
        elif ('self_attn.q_proj' in self_param_name or 'self_attn.k_proj' in self_param_name or \
            'self_attn.v_proj' in self_param_name) and ('bias' in self_param_name):
            ss = self_param_name.split('.')
...
if __name__ == '__main__':
    ...
    fm_models_dict_path = 'new_impl/cv/clip/results/cls_md_wo_fbs.py/20231113/999994-175831-/data/zql/concept-drift-in-edge-projects/UniversalElasticNet/new_impl/cv/clip/cls_md_wo_fbs.py/models/fm_best.pt'
    fm_models_dict_path = save_models_dict_for_init(torch.load(fm_models_dict_path), __file__, 'fm_clip_cls_lora')
    pretrained_md_models_dict_path = 'new_impl/cv/clip/results/cls_md_wo_fbs.py/20231113/999994-175831-/data/zql/concept-drift-in-edge-projects/UniversalElasticNet/new_impl/cv/clip/cls_md_wo_fbs.py/models/md_best.pt'
    md_models_dict = torch.load(pretrained_md_models_dict_path)
    md_models_dict_path = save_models_dict_for_init(md_models_dict, __file__, 'md_clip_cls_pretrained_wo_fbs')
    torch.cuda.set_device(1)
    device = 'cuda'
    
    fm_model = ElasticDNN_clip_OfflineClsFMModel('fm', fm_models_dict_path, device)
    md_model = ElasticDNN_clip_OfflineClsMDModel('md', md_models_dict_path, device)
