...
class FM_to_MD_clip_Util(FM_to_MD_Util):
    def init_md_from_fm_by_reducing_width(self, fm: nn.Module, reducing_width_ratio: int) -> nn.Module:
        ...
        for block_i,block in enumerate(fm_vis.clip.vision_model.encoder.layers):
            set_module(block, 'self_attn', CLIPAttentionPrunable.init_from_exist_self_attn(block.self_attn,config.vision_config,reducing_width_ratio))
        ...
        for block_i, block in enumerate(fm_vis.clip.vision_model.encoder.layers):
            ...
            proj = get_module(block, f'self_attn.out_proj')
            ...
            fc1 = get_module(block, f'mlp.fc1')
            ...
            fc2 = get_module(block, f'mlp.fc2')
...
class ElasticclipUtil(ElasticDNNUtil):
    ...
    def extract_surrogate_dnn_via_samples(self, master_dnn: nn.Module, samples: torch.Tensor, return_detail=False):#产生小模型的步骤
        ...
        for block_i, block in enumerate(boosted_vit.clip.vision_model.encoder.layers):
            # attn = block.attn
            # ff = block.mlp
            ...
            unpruned_indexes_of_layers[f'clip.vision_model.encoder.layers.{block_i}.mlp.fc1.0.weight'] = ff_0_unpruned_indexes
