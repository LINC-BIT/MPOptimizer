    ...
    def run(self, scenario: Scenario, hyps: Dict, collate_fn=None) -> Dict[str, Any]:
        ...
        for iter_index in pbar:
            ...
            if iter_index % hyps['val_freq'] == 0:
                ...
                if 'transform' in hyps.keys():
                    cur_test_batch_dataset = split_dataset(test_dataset, hyps['val_batch_size'], iter_index, transform=hyps['transform'])[0]
                else:
                    cur_test_batch_dataset = split_dataset(test_dataset, hyps['val_batch_size'], iter_index)[0]
                cur_test_batch_dataloader = build_dataloader(cur_test_batch_dataset, hyps['train_batch_size'], hyps['num_workers'], False, False, collate_fn=collate_fn)
                cur_acc = self.models['main'].get_accuracy(cur_test_batch_dataloader)
                # cur_acc = self.models['main'].get_accuracy(test_loader)
                accs += [{
                    'iter': iter_index,
                    'acc': cur_acc
                }]
            ...
            if(len(source_features.size())>2):
                source_features = torch.squeeze(source_features,-1)
                source_features = torch.squeeze(source_features,-1)
        ...
        if 'transform' in hyps.keys():
            cur_test_batch_dataset = split_dataset(test_dataset, hyps['train_batch_size'], iter_index + 1, transform=hyps['transform'])[0]
        else:
            cur_test_batch_dataset = split_dataset(test_dataset, hyps['train_batch_size'], iter_index + 1)[0]
        cur_test_batch_dataloader = build_dataloader(cur_test_batch_dataset, len(cur_test_batch_dataset), hyps['num_workers'], False, False, collate_fn=collate_fn)
        cur_acc = self.models['main'].get_accuracy(cur_test_batch_dataloader)
        accs += [{
            'iter': iter_index + 1,
            'acc': cur_acc
