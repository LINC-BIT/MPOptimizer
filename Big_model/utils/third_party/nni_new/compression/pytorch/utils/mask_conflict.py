...
class GroupMaskConflict(MaskFix):
    ...
    def fix_mask(self):
        ...
        for layername in depens:
            ...
            all_ones = (w_mask.flatten(1).sum(-1) == count).nonzero(as_tuple=False).squeeze(1).tolist()
            all_zeros = (w_mask.flatten(1).sum(-1) == 0).nonzero(as_tuple=False).squeeze(1).tolist()
