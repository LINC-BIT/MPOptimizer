...
def _dummy_debug(*args):
    return None
_logger.debug = _dummy_debug
...
replace_module = {
    ...
    'ShuffleBlock': lambda module, mask: no_replace(module, mask),
    'hswish': lambda module, mask: no_replace(module, mask),
    'hsigmoid': lambda module, mask: no_replace(module, mask),
    'LeakyReLU': lambda module, mask: no_replace(module, mask),
    'ZeroPad2d': lambda module, mask: no_replace(module, mask),
    
    'SiLU': lambda module, mask: no_replace(module, mask),
}
...
def replace_batchnorm2d(norm, mask):
    ...
    index = mask.param_masks['weight'].mask_index[0].to(norm.weight.device)
...
def replace_conv2d(conv, mask):
    ...
    if mask.output_mask is not None:
        tmp_weight_data = torch.index_select(
            conv.weight.data, 0, out_channels_index.to(conv.weight.device))
        if conv.bias is not None:
            tmp_bias_data = torch.index_select(
                conv.bias.data, 0, out_channels_index.to(conv.weight.device))
    else:
...
def replace_convtranspose2d(convtrans, mask):
    ...
    if convtrans.in_channels == convtrans.out_channels == convtrans.groups:
        ...
        assert in_channels == out_channels
        groups = in_channels
    _logger.info('Replace convtranspose2d %s with in_channels:%d out_channels:%d',
                  mask.module_name, in_channels, out_channels)
