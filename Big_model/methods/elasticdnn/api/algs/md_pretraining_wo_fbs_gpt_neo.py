...
class ElasticDNN_MDPretrainingWoFBSAlg(BaseAlg):
    ...
    def run(self, scenario: Scenario, hyps: Dict, collate_fn=None) -> Dict[str, Any]:
        ...
        if self.models['md'].models_dict['main'] == -1:
            ...
            # master_dnn = self.models['fm'].generate_md_by_reducing_layers(hyps['generate_md_layers'], 
            #                                                             sample)
        ...
        for iter_index in pbar:
            ...
            if md_output_hook is None:
                md_output_hook = self.models['md'].get_feature_hooks()
                fm_output_hook = self.models['fm'].get_feature_hooks()
            ...
            distill_loss = self.models['md'].get_distill_loss(md_output_hook, fm_output_hook, x['attention_mask'])
            total_loss = (1 - hyps['distill_loss_weight']) * task_loss + hyps['distill_loss_weight'] * distill_loss
            ...
            if (iter_index + 1) % hyps['val_freq'] == 0:
                ...
                # elastic_dnn_util.clear_cached_channel_attention_in_master_dnn(self.models['md'].models_dict['main'])
                md_output_hook['head'].remove()
                for hidden in md_output_hook['hiddens']:
                    hidden.remove()
                md_output_hook = None
                fm_output_hook['head'].remove()
                for hidden in fm_output_hook['hiddens']:
                    hidden.remove()
                fm_output_hook = None
